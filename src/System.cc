...
namespace ORB_SLAM2 {
    ...
    System::System(const string &strVocFile, const string &strSettingsFile, const eSensor sensor,
        ...
        mptCacheInsertFromServer = new thread(&ORB_SLAM2::Cache::InsertDataFromServer, mpCacher);
    }
    ...
    void System::Shutdown() {
        ...
        mpLocalMapper->RequestFinish(); cout << "mpLocalMapper->RequestFinish..." << endl;
        mpLoopCloser->RequestFinish(); cout << "mpLoopCloser->RequestFinish()" << endl;
        ...
        mpCacher->RequestFinish(); cout << "mpCacher->RequestFinish()..." << endl;
        while(!mpCacher->isFinished())
            usleep(5000);
        
        cout << "send to server finished" << endl;
        
        mpCacher->RequestFinishRunSubFromServer(); cout << "mpCacher->RequestFinishRunSubFromServer()..." << endl;
        while(!mpCacher->isFinishedRunSubFromServer()){
            usleep(5000);
        }        
        cout << "insert from queues finished" << endl;
        //pangolin::BindToContext("ORB-SLAM2: Map Viewer");
        
        if(mpViewer)
            pangolin::BindToContext("ORB-SLAM2: Map Viewer");
        
        //mpCacher->RequestServerReset();
