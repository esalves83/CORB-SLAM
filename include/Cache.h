...
namespace ORB_SLAM2 {
    ...
    public:
        ...
        void RequestFinish();
        void RequestFinishRunSubFromServer();

        bool isFinished();
        bool isFinishedRunSubFromServer();
        ...
        std::set<LightMapPoint> deletedMPs;
    ...
    private:
        ...
        bool CheckFinishRunSubFromServer();
        void SetFinishRunSubFromServer();
        
        void subNewInsertKFs(std::string str);
        void subNewInsertMPs(std::string str);
        void subUpInsertKFs(std::string str);
        void subUpInsertMPs(std::string str);
        ...
        void deletedMapPointsToServer(std::set<LightMapPoint> tdeletedMPs);

        void subNewInsertKeyFramesFromServer(std::string data);
        void subNewInsertMapPointFromServer(std::string data);
        void subUpdatedKeyFramesPose(std::string data);
        void subUpdatedMapPointsPose(std::string data);
    ...
    private:
        ...
        std::mutex mMutexNewKFsQueue;
        std::mutex mMutexNewMPsQueue;
        std::mutex mMutexUpKFsQueue;
        std::mutex mMutexUpMPsQueue;
        
        std::list<std::string> mlNewKFsQueue;
        std::list<std::string> mlNewMPsQueue;
        std::list<std::string> mlUpKFsQueue;
        std::list<std::string> mlUpMPsQueue;

        bool mbFinished;
        bool mbFinishedRunSubFromServer;
        ...
        std::mutex mMutexFinishRunSubFromServer;
        bool mbFinishRequestedRunSubFromServer;
